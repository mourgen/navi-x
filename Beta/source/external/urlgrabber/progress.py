...
class MultiFileMeter:
    ...
    def start(self, freespace, numfiles=None, total_size=None, now=None):
        if now is None: now = time.time()
        self.numfiles         = numfiles
        self.freespace        = freespace
        self.freespace_check  = True
        self.finished_files   = 0
        self.failed_files     = 0
        self.open_files       = 0
    ...
    def _do_update_meter(self, meter, now):
        if self.freespace_check:
            if self.freespace < meter.size and self.freespace != 0:
            ...
            self.freespace_check = False

        elif meter.size > meter.last_amount_read:
            try:
                #Calc Params
                speed =  int( meter.re.ave_rate / 1000 ) 
                perc = ( meter.last_amount_read * 100 ) / meter.size
                left =  int( ( ( meter.size - meter.last_amount_read ) / meter.re.ave_rate ) / 60 ) 

                #Update Navi GUI
                self.app.gui.SetVisible(4000, True)
                self.app.gui.SetTexture(4001, 'download/download-%s.png' % int( round( perc, -1) ) )
                self.app.gui.SetLabel(4002, 'Item: %s' % meter.text)
                self.app.gui.SetLabel(4003, 'Time Left: %s Min' % left)
                self.app.gui.SetLabel(4004, 'Speed: %s kbps' % speed)
                self.app.gui.SetLabel(4005, 'Downloading: %s%%' % perc )
            except:
                pass
